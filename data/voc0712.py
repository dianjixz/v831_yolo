    ...
    def __init__(self, root,
        ...
        for (year, name) in image_sets:
            ...
            for line in open(osp.join(rootpath, 'ImageSets', 'Main', name + '.txt')):
                self.ids_all.append((rootpath, line.strip()))
        self.targets = []
        for i in range(len(self.ids_all)):
            im, gt, h, w = self._pull_item(i)
            if not im is None:
                self.targets.append((im, gt, h, w))
                self.ids.append(self.ids_all[i])
