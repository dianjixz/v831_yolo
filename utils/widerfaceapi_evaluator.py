...
class WiderfaceAPIEvaluator():
    """ VOC AP Evaluation class """
    def __init__(self, data_root, img_size, device, transform, labelmap, set_type='test', display=False, use_horovod=False):
        ...
        self.output_dir = self.get_output_dir('widerface_eval/', self.set_type)
    ...
    def write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.labelmap):
            if self.display:
                print('Writing {:s} widerface results file'.format(cls))
    ...
    def do_python_eval(self, use_07=True):
        ...
        print('widerface metric? ' + ('Yes' if use_07_metric else 'No'))
